...
public class JmxMetricsReporter implements CodahaleReporter {
    ...
    public JmxMetricsReporter(MetricRegistry registry, IronDBContext conf) {
        this.registry = registry;
        this.conf = conf;

        jmxReporter = JmxReporter.forRegistry(registry).convertRatesTo(TimeUnit.SECONDS)
                .createsObjectNamesWith(new ObjectNameFactory()).convertDurationsTo(TimeUnit.MILLISECONDS).build();
    }
