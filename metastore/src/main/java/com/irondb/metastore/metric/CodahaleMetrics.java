...
package com.irondb.metastore.metric;
...
import com.irondb.metastore.reporter.*;
...
public class CodahaleMetrics implements Metrics {
    ...
    public CodahaleMetrics() {
        this.conf = IronDBContext.fromInputStream();
    ...
    private boolean initCodahaleMetricsReporterClasses() {
        ...
        List<String> reporterClasses = Lists.newArrayList(Splitter.on(",").trimResults().omitEmptyStrings()
                .split(IronDBContext.getString("MetricsReportClassList")));
        ...
        for (String reporterClass : reporterClasses) {
            ...
            try {
                Constructor constructor = name.getConstructor(MetricRegistry.class, IronDBContext.class);
                CodahaleReporter reporter = (CodahaleReporter) constructor.newInstance(metricRegistry, conf);
                reporter.start();
                reporters.add(reporter);
            } catch (NoSuchMethodException | InstantiationException | IllegalAccessException
                    | InvocationTargetException e) {
                LOGGER.error("Unable to instantiate using constructor(MetricRegistry, IronDBContext) for" + " reporter "
